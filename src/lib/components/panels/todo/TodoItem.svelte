...
<div class="flex items-center gap-2 px-2.5 py-2 rounded-lg hover:bg-white/[0.03] transition-colors group {todo.completed ? 'opacity-50' : ''}">
  <button
  ...
  <button onclick={cyclePriority} class="text-xs shrink-0 hover:scale-125 transition-transform" title="切换优先级" style="color: {priorityColors[todo.priority]}">
  ...
  <button
    ...
    class="w-4 h-4 rounded flex items-center justify-center opacity-0 group-hover:opacity-100 hover:bg-red-500/30 transition-all text-white/30 hover:text-red-400 shrink-0"
    title="删除"
  >
